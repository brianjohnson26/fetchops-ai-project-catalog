...
async function main() {
  ...
  await Promise.all(tools.map(name => prisma.tool.upsert({
  ...
  const p1 = await prisma.project.create({
    data: {
      ...
      description: "Classify and route Zendesk tickets using an LLM; auto-replies for common intents.",
      team: "Support",
      slackHandle: "@b.johnson",
  ...
  const p2 = await prisma.project.create({
    data: {
      ...
      description: "Detects anomalous receipt behavior and routes to Fraudal with priority scoring.",
      team: "Fraud",
      slackHandle: "@sara.h",
